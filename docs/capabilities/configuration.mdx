...
| `ACP_FOLLOWUP_IDLE_SECS` | Maximum idle time between prompts for interactive ACP or Claude Code jobs (default: 300 seconds). |
...
ACP and Claude Code sandbox jobs created with `wait=true` run one turn and terminate. Jobs created with `wait=false` accept `job_prompt` follow-ups until `done=true`, the agent process exits, or `ACP_FOLLOWUP_IDLE_SECS` elapses without a new prompt.
